...
const UnauthorizeError = require('../helpers/unauthorized-error')

const makeSut = () => {
  class AuthUseCaseSpy { // class mock
    auth (email, password) {
      this.email = email
...
describe('Login Router', () => {
  test('Shoud return 400 if no email is provider', () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
  ...
  test('Shoud return 400 if no password is provider', () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
  ...
  test('Shoud return 500 if AuthUseCase has no auth method', () => {
    const sut = new LoginRouter({})
    const httpRequest = {
      body: {
        email: 'any_email@mail.com',
